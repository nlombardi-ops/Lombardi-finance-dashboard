...
- **Assistente Finanze chat** (`app/api/dashboard/chat/route.ts` +
  `FinanceChat.tsx`) — works as soon as `ANTHROPIC_API_KEY` is set. Builds
  its context by reading all 10 area JSON files live, so it's always current
  with whatever's in `data/`.
